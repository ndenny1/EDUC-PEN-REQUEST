<template>
  <v-card height="100%" width="100%" color="#C3F1E8" class="pa-3" v-if="status === requestStatuses.INITREV || status === requestStatuses.SUBSREV">
  ...
  <v-card height="100%" width="100%" color="#FFECA9" class="pa-3" v-else-if="status === requestStatuses.DRAFT && ! timedout">
    <p class="mb-2"><strong>Your email is not yet verified. Complete the email verification process.</strong></p>
    <ol>
      <li>Go to your email inbox for {{ request.email }} and check for an email from {{ ministry }}. Check your spam folder too.</li>
  ...
  <v-card height="100%" width="100%" color="#FFECA9" class="pa-3" v-else-if="status === requestStatuses.REJECTED || status === requestStatuses.UNMATCHED">
